...
def navigate_clean_menu(config, path=[]):
    current = config
    for key in path:
        current = current[key]
    while True:
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"\n[ CLEAN MENU ] Path: {'/'.join(path) if path else '(root)'}")
        print("Type 'help' to see script usage instructions.\n")
        if isinstance(current, dict):
            keys = list(current.keys())
            for idx, k in enumerate(keys, 1):
            ...
            try:
                ...
                if 1 <= choice <= len(keys):
                    path.append(keys[choice-1])
                    return navigate_clean_menu(config, path)
                elif choice == len(keys)+1:
                    print(">>> Cleaning:", ' '.join([MAKE_DB_SCRIPT, 'clean'] + path))
                    run_make_db("clean", path)
                    input("Press Enter to continue...")
                elif path and choice == len(keys)+2:
                    path.pop()
                    return
            ...
            try:
                choice = int(choice)
                if 1 <= choice <= len(current):
                    print(f">>> Cleaning only: {current[choice-1]}")
                    run_make_db("clean", path + [str(choice-1)])
                    input("Press Enter to continue...")
                elif choice == len(current)+1:
                    print(">>> Cleaning all under this node")
                    run_make_db("clean", path)
                    input("Press Enter to continue...")
                elif path and choice == len(current)+2:
                    path.pop()
...
def main_menu():
    while True:
        ...
        if choice.lower() == "help" or choice == "3":
            ...
            config = load_config()
            navigate_clean_menu(config["clean"])
        elif choice == "4":
            print("Exiting. Have a nice day!")
            break
        else:
