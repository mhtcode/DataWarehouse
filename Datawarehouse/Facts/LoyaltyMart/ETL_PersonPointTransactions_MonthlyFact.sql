CREATE OR ALTER PROCEDURE [DW].[Load_PersonPointTransactions_MonthlyFact]
AS
BEGIN
	SET NOCOUNT ON;
  ...
  IF @StartDate >= @EndDate
	BEGIN
		RAISERROR('The PersonPointTransactions_MonthlyFact table is up to date!', 0, 1) WITH NOWAIT;
		RETURN;
	END
	...
	BEGIN
		...
		BEGIN TRY
			DELETE FROM [DW].[PersonPointTransactions_MonthlyFact] WHERE MonthID = @CurrentMonthStart;
